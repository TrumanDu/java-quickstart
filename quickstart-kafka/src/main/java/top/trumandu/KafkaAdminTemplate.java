...
public class KafkaAdminTemplate implements Closeable {
    ...
    public List<TopicPartitionInfo> listTopicPartition(String topicName) throws Exception {
        return this.adminClient.describeTopics(Collections.singleton(topicName)).topicNameValues().get(topicName).get().partitions();
    }
    ...
    public List<Node> nodes() throws ExecutionException, InterruptedException {
        return new ArrayList<>(this.adminClient.describeCluster().nodes().get());
    }

    /**
     * 根据Topic获取LogSize之和
     */
    public long getLogSize(String topic) {
